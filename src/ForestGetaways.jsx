...
import r1 from './assets/NatureQ/NatureQ19.avif'
import r2 from './assets/NatureQ/NatureQ21.avif'
import r3 from './assets/NatureQ/NatureQ22.avif'
...
export default function ForestGetaways(){
  ...
  const articleParagraphs = [
    "Wake to birdsong and the scent of pine. These routes favor quiet camps and short nature walks ideal for mindful resets.",
    "Pack well and bring a lightweight daypack for short hikes; trail conditions can vary.",
    "Respect campfire rules and check for seasonal closures in sensitive habitats.",
    "Choose established paths to minimize ecological impact — small steps matter.",
    "Plan for damp mornings and keep waterproof layers handy."
  ]
  ...
  return (
    <PageTemplate title="Forest Getaways" subtitle="Campsites Among the Pines" heroNote="Trail maps and fire safety info" recent={recentToShow}>
      ...
      <article className="article-body">
        <h2>Forest Getaways: Trails & tips</h2>
        <div className="byline">September 12, 2025 — <span className="author">NatureQuest</span></div>

        <p className="lead">{highlight(articleParagraphs[0],q)}</p>
        <p>{highlight(articleParagraphs[1],q)}</p>
        <img className="inline-img" src={r1} alt="forest" />
        <p>{highlight(articleParagraphs[2],q)}</p>
        <p className="pull-quote">“Nature calms the mind — plan short hikes and stay present.”</p>
        <img className="inline-img" src={r2} alt="river walk" />
        <p>{highlight(articleParagraphs[3],q)}</p>
        <img className="inline-img" src={r3} alt="pine view" />
        <h3>Trail starters</h3>
